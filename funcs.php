...
function GeneratePage()
{
    ...
    list($top_str, $r) = TopMenu($xml, $s);
    
    $top_str .= "\n" . '<div id="main">' . "\n\n";
    
    list($side_str, $t) = SideMenu($xml, $s, $r, $c);

    $header_str = HeaderStr($xml, $r, $t);
    
    $content_str = ContentStr($xml, $r, $t);
    
    $footer_str = FooterStr();

    echo $header_str . $top_str . $side_str . $content_str . $footer_str;
}

/* s and c should be english smal chars only! die if not: */
...
function HeaderStr($xml, $r, $t)
{
    $str =  '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">' . "\n" .
            ...
            "<body>\n\n" .
            '<div id="container">' . "\n\n" .
            '<div id="header-main"><a href="?s=index">בית הרב</a></div>' ."\n" .
            '<div id="header-sub"><a href="?s=index">חצרות בית ה\' - קול רינה ותורה</a></div>';

    $ttl = "בית הרב - אתר הפיוטים";
    if (isset($r))
        $ttl = $xml->section[$r]->title;
    if (isset($t))
        $ttl .= " - " . $xml->section[$r]->chapters->chapter[$t]->title;
...
function FooterStr()
{
    $str = '<div class="clear"></div>' . "\n\n".
           "</div>\n\n" .
           '<div id="footer">' . "\n" .
           '&copy; 2010  &nbsp;<span class="separator">|</span> &nbsp; ' . "\n" . "כל הזכויות שמורות" . 
           "</div>\n\n</div>\n\n</body>\n</html>\n\n";
